...
**Phase 5 verification (implemented).** `backend/tests/integration_drive_os.py` boots the 5 services as real subprocesses (hermetic — forced SQLite + fixed test secrets, never touches the real DB) and drives the full east-west chain over HTTP with a real HS256 token: **12/12** checks covering evidence append+conflict, decision coverage (67% from the model) / agreement (divergent) / confidence ranking, decision→evidence lineage, action derivation, insights, and interviewer calibration drift.

**Phase 6 (partial).** Evidence immutability now has two layers: the append-only trigger **and** `REVOKE UPDATE, DELETE ... FROM PUBLIC`. RBAC reviewed across all 5 services — every route is role-gated, all writes require an elevated role, no unguarded endpoints.

**Frontend intelligence wiring (implemented).** Candidate Intelligence ranks on real evidence-backed decision confidence (from the decision queue); candidate names resolved across Evidence Ledger / Decision queue / Interviewer Workspace; interviewer **Calibration** panel on the Decisions tab; and the Interviewer Workspace emits **per-competency** interview evidence (competency selector, signal = score×20) feeding decisions + calibration.

**Still future.** True bus-backed **SSE** streaming (deferred deliberately: browser `EventSource` cannot send the RS256 bearer, and long-lived server streams risk worker exhaustion — the safe path is a fetch-stream endpoint + gateway query-token support); a load test on the evidence/action hot path; **per-competency evidence from coding sources** (needs the coding grader to break results down by competency); **cross-drive calibration**; platform-wide **dark mode** (a token-system refactor, not a flag); re-skinning the peripheral candidate screens (My Drive, Matched Opportunities, Exam Portal, Question Bank) into the new grammar.
