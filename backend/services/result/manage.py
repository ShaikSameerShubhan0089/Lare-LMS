...
from app.config import ResultConfig  # noqa: E402
...
def cmd_serve():
    ...
    serve(build_app(), host="127.0.0.1", port=int(os.getenv("PORT", "8019")))


def cmd_run():
    build_app().run(host="127.0.0.1", port=int(os.getenv("PORT", "8019")), debug=True)
