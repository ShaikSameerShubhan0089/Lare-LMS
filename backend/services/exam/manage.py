...
from app.config import ExamConfig  # noqa: E402
...
def cmd_serve():
    ...
    serve(build_app(), host="127.0.0.1", port=int(os.getenv("PORT", "8013")))


def cmd_run():
    build_app().run(host="127.0.0.1", port=int(os.getenv("PORT", "8013")), debug=True)
